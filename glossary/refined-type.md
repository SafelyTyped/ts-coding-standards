...
A _refined type_ is simply a subset of a larger type.

_Type refinement_ is the process of converting a large type to a _refined type_. We use _smart constructors_ to do _type refinement_.

For example:

* all UUIDs are valid strings, but not all strings are valid UUIDs
* UUIDs are a _refined type_ of `string`
* the function `makeUuid(input: string): Uuid`:
  - takes a string
  - makes sure it contains a valid UUID
  - spits out a UUID built from that string

In a business application, many (if not most) of the safe types will actually be _refined types_.
