# 3.3.14 - Default Values Start With DEFAULT_
...
- [Discussion](#discussion)
  - [Make Default Values Easy To Discover](#make-default-values-easy-to-discover)

## Description

All default values must be called `DEFAULT_XXX`.

For example:

```typescript
const DEFAULT_DATA_PATH = "input" as DataPath;
```

## Discussion

### Make Default Values Easy To Discover

It's a lot easier for [end-users][End-User] to quickly find the right default value to plug in if we follow a consistent naming convention.

In their IDE of choice, all they've got to do is type `DEFAULT_`, and they'll see a list available default values.
