# 3.3.4 - Use SCREAMING_SNAKE_CASE For All Constants
...
A _constant_ is a read-only variable that contains something that isn't a function. _Constants_ are declared at the module-level / file-level.

```typescript
// this is an constant
//
// it is declared outside of any class or function
export const MODULE_NAME = makeModuleName("@safelytyped/mediatype/lib/v1");
```

Treat constants declared inside functions as variables.
...
Treat functions that are assigned to constants as functions.

```typescript
// this is a function
const smartConstructor = (x: string): ModuleName => ...;
```

## Discussion

This is the common style used in the Javascript world.
