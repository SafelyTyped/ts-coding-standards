...
- [Discussion](#discussion)
  - [Reducing Noise When Refactoring](#reducing-noise-when-refactoring)

## Description

Every module folder must have an index file.

Use these index files to re-export (possibly a subset of) artefacts from the individual files in the folder.
...
### Reducing Noise When Refactoring

Despite our best endeavours, we won't put items in the right file 100% of the time. We will need to relocate exported artefacts.

When that happens, we want to keep the size of pull-requests down. One way we can do that is to have our own code `import` public API items from index files, instead of from individual module files.
