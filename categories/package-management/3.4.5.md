# 3.4.5 - One Exported Function Per File
...
- [Discussion](#discussion)
  ...
  - [Taking Advantage Of Modules](#taking-advantage-of-modules)

## Description

Put exported functions in their own files.

```typescript
// src/v1/ContentType/makeContentType.ts
export function makeContentType() { ... };

// src/v1/MediaType/makeMediaType.ts
export function makeMediaType() { ... };
```
...
We are putting exported functions into their own files to make it easier to navigate around a source tree.
...
We are putting exported functions into their own files to make code reviews easier.
...
Javascript (and therefore Typescript) is a modular language. Files have their own scope. If we put exported functions into their own files, then those files can include non-exported functions, constants, etc to support the exported function as required.
