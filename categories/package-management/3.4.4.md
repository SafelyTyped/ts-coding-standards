# 3.4.4 - One Exported Class Per File
...
Put exported classes in their own files.

```typescript
// src/v1/ContentType/ContentType.ts
export class ContentType extends RefinedString {

}

// src/v1/MediaType/MediaType.ts
...
We are putting classes into their own files to make it easier to navigate around a source tree.
...
We are putting classes into their own files to make code reviews easier.
...
Javascript (and therefore Typescript) is a modular language. Files have their own scope. If we put exported classes into their own files, then those files can include non-exported functions, constants, etc to support the class as required.
