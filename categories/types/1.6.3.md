...
Every instantiable type must have a type guarantee.
...
A _type guarantee_ is a Typescript function that verifies if an `unknown` input is the expected type. It calls the `onError` handler if the verification fails.

## Discussion

_Type guarantees_ are used at runtime, for those circumstances where we're converting from unsafe types to safe types.

When you create a type, create a _type guarantee_ to go along with it. That way, it's there if anyone ever needs it.
